<script lang="ts">
  ...
  let rooms = [
    ...
    {
      id: "2",
      name: "SuperNode Support",
      lastMsg: "Need help with config",
      time: "11:45",
    },
    {
      id: "3",
      name: "Development",
      lastMsg: "Bug fix deployed",
      time: "Yesterday",
    },
  ];
  ...
  function handleSend() {
    ...
    // Integration with OpenClaw SDK would go here
    messageInput = "";
  }
</script>
...
<div class="flex flex-1 h-full overflow-hidden">
    ...
    <div class="flex-1 overflow-y-auto">
      {#each rooms as room}
        <button
          ...
          class="w-full p-4 flex gap-4 hover:bg-white/5 transition-colors border-l-2 {selectedRoomId ===
          room.id
            ? 'border-nexus-accent bg-white/5'
            : 'border-transparent'}"
        >
          <div
            class="w-12 h-12 rounded-xl bg-nexus-card border border-white/10 flex items-center justify-center text-xl"
          >
            {room.name[0]}
          </div>
  ...
  <div class="flex-1 flex flex-col min-w-0 bg-nexus-bg">
    {#if selectedRoomId}
      ...
      <div class="flex-1 p-6 overflow-y-auto space-y-6">
        <div class="flex gap-4">
          ...
          <div class="space-y-1">
            ...
            <div
              class="bg-nexus-card border border-white/10 p-4 rounded-2xl rounded-tl-none max-w-lg"
            >
        ...
        <div class="flex gap-4 justify-end">
          <div class="space-y-1 items-end flex flex-col">
            ...
            <div
              class="bg-nexus-accent text-black p-4 rounded-2xl rounded-tr-none max-w-lg font-medium"
            >
              Thanks! The OpenClaw integration looks solid.
            </div>
          </div>
      ...
      <footer class="p-6">
        <div
          class="bg-white/5 border border-white/10 rounded-2xl p-2 flex items-center gap-2"
        >
          <button
            class="w-10 h-10 rounded-xl hover:bg-white/5 flex items-center justify-center text-xl"
            >📎</button
          >
          <input
            type="text"
